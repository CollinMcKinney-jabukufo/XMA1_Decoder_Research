/*
 * XMA2 compatible decoder
 * Copyright (c) 2007 Baptiste Coudurier, Benjamin Larsson, Ulion
 * Copyright (c) 2008 - 2011 Sascha Sommer, Benjamin Larsson
 * Copyright (c) 2015 - Xenia
 *
...
// This file is the WMA Pro decoder repurposed for XMA2 decoding.
...
#define WMAPRO_BLOCK_MIN_SIZE \
  (1 << WMAPRO_BLOCK_MIN_BITS)  ///< minimum block size
#define WMAPRO_BLOCK_MAX_SIZE \
  (1 << WMAPRO_BLOCK_MAX_BITS)  ///< maximum block size
#define WMAPRO_BLOCK_SIZES \
  (WMAPRO_BLOCK_MAX_BITS - WMAPRO_BLOCK_MIN_BITS + 1)  ///< possible block sizes
...
typedef struct XMA2ChannelCtx {
  ...
  DECLARE_ALIGNED(32, float, out)
  [WMAPRO_BLOCK_MAX_SIZE + WMAPRO_BLOCK_MAX_SIZE / 2];  ///< output buffer
} XMA2ChannelCtx;

/**
 * @brief channel group for channel transformations
 */
typedef struct XMA2ChannelGrp {
...
typedef struct XMA2DecodeCtx {
  ...
  DECLARE_ALIGNED(32, float, tmp)
  [WMAPRO_BLOCK_MAX_SIZE];  ///< IMDCT output buffer
  float*
      windows[WMAPRO_BLOCK_SIZES];  ///< windows for the different block sizes
  ...
  XMA2ChannelCtx channel[WMAPRO_MAX_CHANNELS];  ///< per channel data
} XMA2DecodeCtx;
 ...
 *@param s context
 */
static av_cold void dump_context(XMA2DecodeCtx* s) {
...
static av_cold int decode_end(AVCodecContext* avctx) {
  ...
  for (i = 0; i < WMAPRO_BLOCK_SIZES; i++) ff_mdct_end(&s->mdct_ctx[i]);
...
static av_cold int decode_init(AVCodecContext* avctx) {
  ...
  if (avctx->channels < 0) {
    ...
    avpriv_request_sample(avctx, "More than %d channels", WMAPRO_MAX_CHANNELS);
    return AVERROR_PATCHWELCOME;
  }
  ...
  INIT_VLC_STATIC(&sf_vlc, SCALEVLCBITS, HUFF_SCALE_SIZE, scale_huffbits, 1, 1,
                  scale_huffcodes, 2, 2, 616);

  INIT_VLC_STATIC(&sf_rl_vlc, VLCBITS, HUFF_SCALE_RL_SIZE, scale_rl_huffbits, 1,
                  1, scale_rl_huffcodes, 4, 4, 1406);

  INIT_VLC_STATIC(&coef_vlc[0], VLCBITS, HUFF_COEF0_SIZE, coef0_huffbits, 1, 1,
                  coef0_huffcodes, 4, 4, 2108);

  INIT_VLC_STATIC(&coef_vlc[1], VLCBITS, HUFF_COEF1_SIZE, coef1_huffbits, 1, 1,
                  coef1_huffcodes, 4, 4, 3912);

  INIT_VLC_STATIC(&vec4_vlc, VLCBITS, HUFF_VEC4_SIZE, vec4_huffbits, 1, 1,
                  vec4_huffcodes, 2, 2, 604);

  INIT_VLC_STATIC(&vec2_vlc, VLCBITS, HUFF_VEC2_SIZE, vec2_huffbits, 1, 1,
                  vec2_huffcodes, 2, 2, 562);

  INIT_VLC_STATIC(&vec1_vlc, VLCBITS, HUFF_VEC1_SIZE, vec1_huffbits, 1, 1,
  ...
  for (i = 0; i < num_possible_block_sizes; i++) {
    ...
    for (x = 0; x < MAX_BANDS - 1 && s->sfb_offsets[i][band - 1] < subframe_len;
         x++) {
      int offset =
          (subframe_len * 2 * critical_freq[x]) / s->avctx->sample_rate + 2;
      offset &= ~3;
  ...
  for (i = 0; i < num_possible_block_sizes; i++) {
    ...
    for (b = 0; b < s->num_sfb[i]; b++) {
      int x;
      int offset =
          ((s->sfb_offsets[i][b] + s->sfb_offsets[i][b + 1] - 1) << i) >> 1;
      for (x = 0; x < num_possible_block_sizes; x++) {
        int v = 0;
        while (s->sfb_offsets[x][v + 1] << x < offset)
          if (++v >= MAX_BANDS) return AVERROR_INVALIDDATA;
  ...
  for (i = 0; i < 33; i++) sin64[i] = sin(i * M_PI / 64.0);

  if (avctx->debug & FF_DEBUG_BITSTREAM) dump_context(s);
 ...
 */
static int decode_subframe_length(XMA2DecodeCtx* s, int offset) {
 ...
 */
static int decode_tilehdr(XMA2DecodeCtx* s) {
  uint16_t num_samples[WMAPRO_MAX_CHANNELS] = {
      0}; /**< sum of samples for all currently known subframes of a channel */
  uint8_t contains_subframe[WMAPRO_MAX_CHANNELS];     /**< flag indicating if a
                                                         channel contains the
                                                         current subframe */
  int channels_for_cur_subframe = s->avctx->channels; /**< number of channels
                                                         that contain the
                                                         current subframe */
  int fixed_channel_layout = 0; /**< flag indicating that all channels use the
                                   same subframe offsets and sizes */
  int min_channel_len = 0;      /**< smallest sum of samples (channels with this
                                   length will be processed first) */
  int c;
  ...
  for (c = 0; c < s->avctx->channels; c++) s->channel[c].num_subframes = 0;
  ...
  do {
    ...
    for (c = 0; c < s->avctx->channels; c++) {
      if (num_samples[c] == min_channel_len) {
        if (fixed_channel_layout || channels_for_cur_subframe == 1 ||
            (min_channel_len ==
             s->samples_per_frame - s->min_samples_per_subframe))
    ...
    for (c = 0; c < s->avctx->channels; c++) {
      XMA2ChannelCtx* chan = &s->channel[c];
 ...
 */
static void decode_decorrelation_matrix(XMA2DecodeCtx* s,
                                        XMA2ChannelGrp* chgroup) {
  int i;
  int offset = 0;
  int8_t rotation_offset[WMAPRO_MAX_CHANNELS * WMAPRO_MAX_CHANNELS];
  memset(chgroup->decorrelation_matrix, 0,
         s->avctx->channels * s->avctx->channels *
 ...
 */
static int decode_channel_transform(XMA2DecodeCtx* s) {
  int i;
  ...
  if (s->avctx->channels > 1) {
    ...
    if (get_bits1(&s->gb)) {
      avpriv_request_sample(s->avctx, "Channel transform bit");
      return AVERROR_PATCHWELCOME;
    }
    ...
    for (s->num_chgroups = 0;
         ...
         s->num_chgroups++) {
      XMA2ChannelGrp* chgroup = &s->chgroup[s->num_chgroups];
      ...
      if (chgroup->num_channels == 2) {
        if (get_bits1(&s->gb)) {
          if (get_bits1(&s->gb)) {
            avpriv_request_sample(s->avctx, "Unknown channel transform type");
            return AVERROR_PATCHWELCOME;
          }
        } else {
          chgroup->transform = 1;
        ...
        if (get_bits1(&s->gb)) {
          ...
          if (get_bits1(&s->gb)) {
            ...
            if (chgroup->num_channels > 6) {
              avpriv_request_sample(s->avctx, "Coupled channels > 6");
            } else {
 ...
 */
static int decode_coeffs(XMA2DecodeCtx* s, int c) {
  ...
  if (cur_coeff < s->subframe_len) {
    ...
    if (ff_wma_run_level_decode(s->avctx, &s->gb, vlc, level, run, 1,
 ...
 *@return 0 on success, < 0 in case of bitstream errors
 */
static int decode_scale_factors(XMA2DecodeCtx* s) {
  int i;
  ...
  for (i = 0; i < s->channels_for_cur_subframe; i++) {
    ...
    s->channel[c].scale_factors =
        s->channel[c].saved_scale_factors[!s->channel[c].scale_factor_idx];
    ...
    if (!s->channel[c].cur_subframe || get_bits1(&s->gb)) {
      if (!s->channel[c].reuse_sf) {
        ...
        for (sf = s->channel[c].scale_factors; sf < sf_end; sf++) {
          val +=
              get_vlc2(&s->gb, sf_vlc.table, SCALEVLCBITS, SCALEMAXDEPTH) - 60;
          *sf = val;
        }
      } else {
        int i;
 ...
 *@param s codec context
 */
static void inverse_channel_transform(XMA2DecodeCtx* s) {
  int i;
  ...
  for (i = 0; i < s->num_chgroups; i++) {
    if (s->chgroup[i].transform) {
      ...
      for (sfb = s->cur_sfb_offsets; sfb < s->cur_sfb_offsets + s->num_bands;
           sfb++) {
        int y;
        if (*tb++ == 1) {
          ...
          for (y = sfb[0]; y < FFMIN(sfb[1], s->subframe_len); y++) {
            ...
            for (ch = ch_data; ch < ch_end; ch++) *data_ptr++ = (*ch)[y];
          ...
          s->fdsp.vector_fmul_scalar(ch_data[0] + sfb[0], ch_data[0] + sfb[0],
                                     181.0 / 128, len);
          s->fdsp.vector_fmul_scalar(ch_data[1] + sfb[0], ch_data[1] + sfb[0],
                                     181.0 / 128, len);
        }
      }
    }
 ...
 *@param s codec context
 */
static void wmapro_window(XMA2DecodeCtx* s) {
  int i;
  for (i = 0; i < s->channels_for_cur_subframe; i++) {
    int c = s->channel_indexes_for_cur_subframe[i];
    float* window;
 ...
 */
static int decode_subframe(XMA2DecodeCtx* s) {
  ...
  */
  ...
  ff_dlog(s->avctx, "processing subframe with offset %i len %i\n", offset,
          subframe_len);
  ...
  if (transmit_coeffs) {
    ...
    if (s->channels_for_cur_subframe == 1) {
      s->channel[s->channel_indexes_for_cur_subframe[0]].quant_step =
          quant_step;
    } else {
  ...
  if (transmit_coeffs) {
    FFTContext* mdct =
        &s->mdct_ctx[av_log2(subframe_len) - WMAPRO_BLOCK_MIN_BITS];
    ...
    for (i = 0; i < s->channels_for_cur_subframe; i++) {
      ...
      if (c == s->lfe_channel)
        memset(&s->tmp[cur_subwoofer_cutoff], 0,
               sizeof(*s->tmp) * (subframe_len - cur_subwoofer_cutoff));
      ...
      for (b = 0; b < s->num_bands; b++) {
        ...
        s->fdsp.vector_fmul_scalar(s->tmp + start, s->channel[c].coeffs + start,
                                   quant, end - start);
      }
 ...
 *        0 if the trailer bit indicates that this is the last frame,
 *        1 if there are additional frames
 */
static int decode_frame(XMA2DecodeCtx* s, AVFrame* frame, int* got_frame_ptr) {
  ...
  if (s->avctx->channels > 1 && get_bits1(gb)) {
    if (get_bits1(gb)) {
      for (i = 0; i < avctx->channels * avctx->channels; i++) skip_bits(gb, 4);
    }
  }
  ...
  for (i = 0; i < avctx->channels; i++) {
    ...
    memcpy(&s->channel[i].out[0], &s->channel[i].out[s->samples_per_frame],
 ...
 */
static int remaining_bits(XMA2DecodeCtx* s, GetBitContext* gb) {
 ...
 */
static void save_bits(XMA2DecodeCtx* s, GetBitContext* gb, int len,
                      int append) {
  int buflen;
  ...
  *got_frame_ptr = 0;
  ...
  if (s->packet_done || s->packet_loss) {
    ...
    if (num_bits_prev_frame > 0) {
      ...
      if (!s->packet_loss) decode_frame(s, (AVFrame*)data, got_frame_ptr);
    ...
    if (s->len_prefix && remaining_bits(s, gb) > s->log2_frame_size &&
      ...
      s->packet_done = !decode_frame(s, (AVFrame*)data, got_frame_ptr);
...
int xma2_decode_frame(AVCodecContext* avctx, AVPacket* avpkt, AVFrame* frame,
  ...
  *got_frame_ptr = 0;
  ...
  /** The user can choose to submit a buffer without a packet header. */
  if (packet_header) {
  ...
  *frame_size = get_frame_length(buf, buf_size, bit_offset);
  ...
  s->packet_done = 0;
  s->packet_loss = 0;
  res = decode_frame(s, frame, got_frame_ptr);
  if (res < 0 || s->packet_loss) {
...
AVCodec ff_xma2_decoder = {
    ...
    .priv_data_size = sizeof(XMA2DecodeCtx),
    .init = decode_init,
    .close = decode_end,
